  ...
  while (1)
  {
    ...
    default:
    {
      if (background)
      {
        ...
        // strncpy(new_job.full_command, line, NL);                // Remove the trailing '&' and newline from the command
        ...
        // Copy the full command and trim trailing characters

        char buffer[256];
        if (v[0]!= NULL && v[1]!= NULL){
          snprintf(buffer, sizeof(buffer), "%s %s",v[0], v[1]);
        }

        strncpy(new_job.full_command, buffer, NL - 1);
        new_job.full_command[strcspn(new_job.full_command, "\n")] = '\0'; // Remove trailing newline

        // Remove trailing '&' if present
        size_t len = strlen(new_job.full_command);
        if (len > 0 && new_job.full_command[len - 1] == '&')
        {
          new_job.full_command[len - 1] = '\0';
        }
